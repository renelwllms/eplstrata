...
export default async function InvoiceTemplateDetailPage({ params }: { params: { id: string } }) {
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Link href="/app/invoices/templates" className="rounded-full bg-ink-900 px-4 py-2 text-sm font-semibold text-sand-50">
            Back
          </Link>
          <DeleteButton
            endpoint={`/api/invoice-templates/${template.id}`}
            confirmText="Delete this template?"
            redirectTo="/app/invoices/templates"
          />
        </div>
      </div>
